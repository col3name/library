  ...
  if (!styleMedia) {
    ...
    styleMedia = {
      matchMedium: function (media) {
        var text = '@media ' + media + '{ #matchmediajs-ajax { width: 1px; } }';
 ...
 * @function
 * @param {jQuery} element - jQuery object to ajax for collisions.
...
/**
 * Runs a callback function when image are fully loaded.
 * @param {Object} images - Image(s) to check if loaded.
 * @param {Func} callback - Function to execute when upload is fully loaded.
 */
function onImagesLoaded(images, callback) {
  ...
  images.each(function () {
    // Check if upload is loaded
    if (this.complete && this.naturalWidth !== undefined) {
      singleImageLoaded();
    } else {
      // If the above check failed, simulate loading on detached element.
      var image = new Image();
      // Still count upload as loaded if it finalizes with an error.
      var events = "load.zf.image error.zf.images";
 ...
 * @requires foundation.util.keyboard
 * @requires foundation.util.imageLoader if tabs contain image
 */
...
var Abide = function (_Plugin) {
  ...
  _createClass(Abide, [{
    ...
    value: function validateText($el, pattern) {
      ...
      if (inputText.length) {
        // If the pattern attribute on the element is in Abide's list of patterns, then ajax that regexp
        if (this.options.patterns.hasOwnProperty(pattern)) {
          valid = this.options.patterns[pattern].test(inputText);
        }
        // If the pattern name isn't also the type attribute of the field, then ajax it as a regexp
 ...
 * @requires foundation.util.imageLoader if equalizer contains image
 */
...
var Interchange = function (_Plugin) {
  ...
  _createClass(Interchange, [{
    ...
    /**
     * Update the `src` property of an upload, or change the HTML of a container, to the specified path.
     * @function
     * @param {String} path - Path to the upload or HTML partial.
